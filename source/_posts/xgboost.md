...
对每个叶子节点j，与j有关的目标函数的部分为$G_jw_j+\frac{1}{2}(H_j+\lambda)w_j^2$. 因为叶子节点相互独立，每个叶子节点对应的目标函数达到最小值，整个目标函数$Obj^{(t)}$就达到最小值。可求得节点j的最优权重$w^*_j=-\frac{G_j}{H_j+\lambda}$，树$f_t$的目标函数值（对**树结构$f_t$打分**）为
$$
...
xgboost梯度的维度？不同样本的梯度是分开求的，每个样本求一次和二次导数，所以维度是：（样本数，特征数\*2）
...
- **导数信息：**GBDT在模型训练时只使用了代价函数的一阶导数信息，xgboost对代价函数进行二阶泰勒展开，同时使用一阶和二阶导数。
